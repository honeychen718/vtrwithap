...
void CongEstBB::InitDemand() {
    noNeedRoute = 0;
    for (auto net : database.clbnets) {
        Box<int> bbox;
        for (auto site : net->sites) bbox.update(site->x, site->y);
        double totW = bbox.hp() + 1;
        unsigned nPins =database.getNet(net->name)->pins.size();
        if (nPins < 10)
            totW *= 1.06;
        else if (nPins < 20)
            totW *= 1.2;
        ...
        int numGCell = (bbox.x() + 1) * (bbox.y() + 1);
        double indW = totW / numGCell;
        siteDemand = vector<vector<double>>(db::database.sitemap_nx, vector<double>(db::database.sitemap_ny, 0));
        for (int x = bbox.lx(); x <= bbox.ux(); ++x)
            for (int y = bbox.ly(); y <= bbox.uy(); ++y) siteDemand[x][y] += indW;
    }
}