...
void gp_copy_group_in(vector<Group> &groups) {
    ...
    for(int x=0; x < database.sitemap_nx; x++){
        for(int y=0;y<database.sitemap_ny;y++){
            if(database.sites[x][y]){
                for(int i=0; i < numFences; i++){
                    if(regions[i].type == database.sites[x][y]->type){
                        FRect rect(x, y, x + 1, y+1);
                        int nRects=regions[i].rects.size();
                        if(nRects == 0 || regions[i].rects[nRects - 1].hy != y){
                            regions[i].addRect(rect);
                        }else{
                            regions[i].rects[nRects - 1].hy = y + 1;
                        }
                        break;
                    }
                }
    ...
    for(FRegion &region : regions){
        vector<FRect> &rects=region.rects;
        for(int i=rects.size()-1;i>=0;i--){
            for(int j=i-1;j>=0;j--){
                if(rects[i].ly==rects[j].ly&&rects[i].hy==rects[j].hy){
                    if(rects[i].lx==rects[j].hx){
                        rects[j].hx=rects[i].hx;
...
double hpwl(double *wx, double *wy) {
    ...
    if (wx != NULL) *wx = totalHPWLX;
    if (wy != NULL) *wy = totalHPWLY;
    return totalHPWLX + totalHPWLY;
}
