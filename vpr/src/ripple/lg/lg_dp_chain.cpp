...
bool ChainFinder::DFS(LGData& lgData, ChainmoveTarget optTarget) {
    ...
    while (true) {
        ...
        if (optTarget == DISP_OPT) {
            double orgDisp = abs((int)groups[gid2Move].x - (int)lgData.groupsX[gid2Move]) +
        ...
        if (doBt) {
            maxTotDisp -= abs((int)groups[gid2Move].x - (int)lgData.groupsX[gid2Move]) +
                          abs((int)groups[gid2Move].y - (int)lgData.groupsY[gid2Move]);
            Backtrack(gid2Move);
            return false;
        }
        ...
        for (auto site : sites) {
            ...
            while (rand() % 100 > 15) {
                ...
                if (canSub) {
                    ...
                    orderChain.push_back(lgorder);
                    posChain.push_back(x * database.sitemap_ny + y);
                    maxTotDisp += (abs((int)groups[gid2Sub].x - (int)lgData.groupsX[gid2Sub]) +
                                   abs((int)groups[gid2Sub].y - (int)lgData.groupsY[gid2Sub])) -
                                  (abs((int)groups[gid2Move].x - x) + abs((int)groups[gid2Move].y - y));
...
void LGData::ChainUpdate(vector<int>& path, vector<VPR_CLB*>& clbChain, vector<vector<int>>& lgorderChain) {
...
bool ChainFinder::Init(Group& group, LGData& lgData) {
    ...
    gidChain = {group.id};
    maxTotDisp = maxDisp =
        abs((int)group.x - (int)lgData.groupsX[group.id]) + abs((int)group.y - (int)lgData.groupsY[group.id]);
...
void DPBle::ChainUpdate(vector<int>& path, vector<VPR_CLB*>& clbChain, vector<vector<int>>& lgorderChain) {
    int destX = path.back() / database.sitemap_ny, destY = path.back() % database.sitemap_ny;
