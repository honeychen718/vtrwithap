...
int groupShifts[4] = {0, 1, 10, 11};

void CLB2::GetResult(Group& group) {
    auto& res = group.instances;
    int numinatinclb=db::database.getSiteType(SiteType::NameString2Enum("SLICE"))->resources.size();
    res.assign(numinatinclb, nullptr);
    if (IsEmpty()) return;
#ifdef USE_LUTPG
    LUTs2.CommitFast();
    LUTs2.Export(LUTs);
